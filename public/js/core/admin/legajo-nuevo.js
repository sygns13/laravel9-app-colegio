...
createApp({
    data() {
        return {
            tituloHeader: "Legajo",
            subtituloHeader: "Registrar Nuevo",
    ...
    methods: {
        changeTipoDoc: function() {
            this.tipoDocumentos.forEach((element) => {
                if(element.id == this.directorEdit.tipo_documento_id){
                    this.tipoDocSelect = element;
                }
            });
            //this.directorEdit.num_documento = '';
        },
        ...
        procesar:function () {
            swal.fire({
                title: '¿Estás seguro?',
                text: "Desea Confirmar la Actualización de los Datos ",
                icon: 'info',
        ...
        updateData: function () {
            ...
            var url="relegajo-update-director/"+this.director.id;
            $(".editClass").attr("disabled");
            this.divloaderEdit=true;

            axios.put(url, this.directorEdit).then(response=>{

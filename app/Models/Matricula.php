...
class Matricula extends Model
{
    ...
    public static function GetAllDataTutorActivo($ciclo_id, $iduser){
        ...
        if($ciclo){
            ...
            foreach ($niveles as $key => $value) {
                $grados = DB::select("select cg.id, cg.orden, cg.nombre, cg.ciclo_niveles_id, cs.tutor_id
                from ciclo_seccion as cs
                inner join ciclo_grados cg on cs.ciclo_grados_id=cg.id
                where cs.tutor_id = ?
                and cg.ciclo_escolar_id = ?
                and cg.ciclo_niveles_id = ?
                ...
                foreach ($grados as $keyG => $valueG) {
                        ...
                        $seccions = DB::select("select cs.id, cs.sigla, cs.nombre, cs.ciclo_grados_id 
                        from ciclo_seccion as cs
                        where cs.tutor_id = ?
                        and cs.ciclo_escolar_id = ?
                        and cs.ciclo_grados_id = ?
